...
- 成长中枢继续只读原201、401背书路径；其他班级路径不公开读取。
- 只有带 `teacher=true` 声明的登录账号可以写入201、401，未知班级即使是教师账号也不能写入。
- 2026-07-23已部署写权限规则并验证匿名写返回 `Permission denied`；教师最小写入探针因当前管理账号缺少 `iam.serviceAccounts.signBlob` 权限而未执行，不能把这一项写成已验收。
